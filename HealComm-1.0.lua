			...
			local hit = false
				...
				local raidId = group[i]
					if raidId then
						...
						if
							-- Case 1: Mouseover is friendly and is rid or rid_pet
							(UnitExists("mouseover") and not UnitCanAttack("player", "mouseover") and UnitIsFriend("player", "mouseover") and
							 (UnitIsUnit("mouseover", rid) or UnitIsUnit("mouseover", rid_pet)))
							or
							-- Case 2: No mouseover, but target is friendly and is rid or rid_pet
							(not UnitExists("mouseover") and UnitExists("target") and not UnitCanAttack("player", "target") and UnitIsFriend("player", "target") and
							 (UnitIsUnit("target", rid) or UnitIsUnit("target", rid_pet)))
							or
							-- Case 3: Mouseover is an enemy, opposing faction, or doesn't exist, and target is not friendly or doesn't exist, so use player if player is rid
							((UnitExists("mouseover") and (UnitCanAttack("player", "mouseover") or not UnitIsFriend("player", "mouseover"))) or not UnitExists("mouseover"))
							and
							((not UnitExists("target") or UnitCanAttack("player", "target") or not UnitIsFriend("player", "target")))
							and
							UnitIsUnit("player", rid)
						then
							hit = true
						end
					else
